...
async function onSaveTrip() {
    ...
    try {
        ...
        // Sua API retorna error "trip_already_exists" com 409
        if (String(e.message).includes("trip_already_exists") || String(e.message).includes("409")) {
            setTripStatus("Já existe lançamento para esse carro nesse dia.\nMarque 'Sobrescrever' e salve novamente para editar.");
            return;
        }
        setTripStatus(`Erro: ${e.message}`);
    }
...
async function onGetStatement() {
  const conn = getConnFromInputs();
  const weekId = $("weekId").value.trim();
  const out = await apiRequest(conn, `/week/${encodeURIComponent(weekId)}/statement`, { method: "GET" });
  $("statementBox").value = out.statementText || "";
  setTripStatus("Extrato gerado.");
}

async function copyStatement() {
  const text = $("statementBox").value || "";
  await navigator.clipboard.writeText(text);
  setTripStatus("Extrato copiado.");
}
...
function init() {
    ...
    $("btnGetStatement").onclick = () => onGetStatement().catch(e => setTripStatus(`Erro: ${e.message}`));
    $("btnCopyStatement").onclick = () => copyStatement().catch(e => setTripStatus(`Erro: ${e.message}`));

    // sugere uma semana (placeholder)
    if (!$("weekId").value) $("weekId").value = "2026-02-02";
